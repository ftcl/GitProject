...
Module
	...
	.controller('TechMyCommentCtrl', ['$scope', '$http', '$ionicPopup',
		function($scope, $http, $ionicPopup) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				//注册handler
				window.WebViewJavascriptBridge.callHandler('CallGetTokenJi', {}, function(response) {
					var param = {
						token: response
					}
					$http({
						method: 'POST',
						url: 'http://sh.plottwists.com/home/MyCommonJi',
						data: $.param(param),
						headers: {
							'Content-Type': 'application/x-www-form-urlencoded'
						}
						...
						if (response.flag == 1) {
							$scope.comments = changeTime(response.message);
						}
					});
				})
				...
				$scope.Call = function(commonId,index) {
					var myPopup = $ionicPopup.confirm({
						...
						if (!res) {
							...
							window.WebViewJavascriptBridge.callHandler('CallGetTokenJi', {}, function(response) {
								var param = {
									token: response,
									commonId:commonId,
									content:$scope.addcomment.content
								}
								$http({
									method: 'POST',
									url: 'http://sh.plottwists.com/home/HuiFu',
									data: $.param(param),
									headers: {
										'Content-Type': 'application/x-www-form-urlencoded'
									}
									...
									if (response.flag == 1) {
										$scope.comments[index].reply = $scope.addcomment.content;
									}
								});
							})
						}
	...
	.controller('MyCommentCtrl', ['$scope', '$http',
		function($scope, $http) {
			window.onerror = function(err) {
				log('window.onerror: ' + err)
			};
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				window.WebViewJavascriptBridge.callHandler('CallGetTokenYong', {}, function(response) {
					var param = {
						token: response
					}
					$http({
						method: 'POST',
						url: 'http://sh.plottwists.com/home/MyCommonYong',
						data: $.param(param),
						headers: {
							'Content-Type': 'application/x-www-form-urlencoded'
						}
