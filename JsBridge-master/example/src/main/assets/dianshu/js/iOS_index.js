			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				bridge.registerHandler('testJavascriptHandler', function(data, responseCallback) {
					$scope.$apply(function() {
						...
						bridge.callHandler('AmountHandler', {}, function(response) {
							$scope.$apply(function() {
								$scope.usermoney = parseInt(response);
							})
						});
						bridge.callHandler('getCouponsHandler', {}, function(responsedata) {
							$scope.$apply(function() {
								var couponsArr = [];
				...
				$scope.Submit = function() {
					...
					if ($scope.usermoney < oldprice && $scope.pay.type == "amount") {
						bridge.callHandler('TipCallback', "余额不足,请选择其他支付方式", function(response) {})
						return false;
					}
					var orderId = dateFilter(new Date(), "yyyyMMddhhmmss") + $scope.severId;

					bridge.callHandler('payBtnHandler', {
	...
	.controller('OrderDetailCtrl', ['$scope', '$http',
		function($scope, $http) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.Comment = function() {
					bridge.callHandler('getTokenHandler', {}, function(callback) {
						var param = {
							token: callback,
						...
						$http({
							...
							if (response.flag == 1) {
								...
								bridge.callHandler('TipCallback', "评论成功", function(tipresponse) {

								})
							} else {
								bridge.callHandler('TipCallback', "评论失败", function(tipresponse) {
		...
		function($scope) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.Comment = function() {
					bridge.callHandler('techDetailCommentCallback', {
						'techDetail': 'A00001'
					}, function(response) {
				...
				$scope.Order = function(severName, Id, price, severPic) {
					bridge.callHandler('techDetailOrderCallback', {
						'name': $scope.username,
						'severName': severName,
						'severId': Id,
						'price': price,
		...
		function($scope) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.ProjectOrder = function() {
					bridge.callHandler('techProjectOrderCallback', {
						'name': $scope.username,
				...
				$scope.otherSever = function() {
					bridge.callHandler('otherSeverHandler', {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.SelectTime = function() {
					if ($scope.checkTime) {
						...
						bridge.callHandler('SelectTimeCallback', {
							'day': day,
						...
						bridge.callHandler('TipCallback', "请选择预约时间", function(response) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.SubmitOrder = function() {
					...
					if (check == true) {
						...
						bridge.callHandler('SubmitOrderCallback', data, function(response) {

						})
					} else {
						bridge.callHandler('TipCallback', check, function(response) {

						})
					}
				}
				$scope.CheckAddress = function() {
					bridge.callHandler('CheckAddressCallback', {
						'CheckAddress': 'A00001'
					}, function(response) {
				...
				$scope.CheckTime = function() {
					bridge.callHandler('CheckTimeCallback', {
						'CheckTime': 'A00001'
					}, function(response) {
		...
		function($scope) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.GetCoupons = function() {
					bridge.callHandler('GetCouponsCallback', {
						'GetCoupons': 'A00001'
					}, function(response) {
				...
				$scope.NowUse = function(couponId) {
					bridge.callHandler('NowUseCallback', {
						'couponId': couponId
					}, function(response) {
			...
			$scope.UseCoupons = function() {
				bridge.callHandler('couponsCallback', {
					'UseCoupons': '11111'
				}, function(response) {
	...
	.controller('TechMyCommentCtrl', ['$scope',
		function($scope) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.Scan = function() {
					bridge.callHandler('ScanCallback', {
						'Scan': '11111'
					}, function(response) {
				...
				$scope.Call = function() {
					bridge.callHandler('CallCallback', {
						'Call': '2222'
					}, function(response) {
		...
		function($scope) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.ProjectDetail = function() {
					bridge.callHandler('projectOrderCallback', {
						'ProjectOrder': 'A00001'
					}, function(response) {
		...
		function($scope, $filter, $location, $ionicScrollDelegate) {
			...
			connectWebViewJavascriptBridge(function(bridge) {
				...
				$scope.selectCity = function(city) {
					bridge.callHandler('SelectCityCallback', {
						'city': city
					}, function(response) {
